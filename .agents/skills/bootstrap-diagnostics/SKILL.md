...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `dispatch-axi`, bootstrap requires it because firstmate reads its ranked output before resolving every crew-dispatch profile array; without it, report the missing requirement and do not choose around an unexamined candidate.
  It has no published package, so its printed install is an editable install of the local project clone in this home; if that clone is absent, the missing project is the real blocker to report.
  For `quota-axi`, bootstrap requires it alongside `dispatch-axi`, which reads it for every window provider, and because `quota-axi auth --json` is firstmate's per-provider credential surface; this also covers an installed build older than the floor.
