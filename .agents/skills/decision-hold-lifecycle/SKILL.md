...
Scout teardown enforces this completion gate: `bin/fm-teardown.sh` calls the script's read-only `verify` subcommand and refuses to remove source state until the inventory exists, so a skipped inventory surfaces as a teardown refusal rather than as silently lost decisions.
