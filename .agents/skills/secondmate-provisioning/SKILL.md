...
The same placement-specific launch and locked bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/crew-limit`, `config/backlog-backend`, `config/backend`, `config/herdr-presentation-spaces`, `config/startup-memory-budget`, and the one shared captain-preference file `data/captain-shared.md`.
`config/trace-context` is declared inherited material too, but it is session-scoped and therefore transfers only at launch convergence, while live convergence leaves a running home's copy untouched ([`docs/configuration.md`](../../../docs/configuration.md) "Trace context propagation" owns that behavior).
