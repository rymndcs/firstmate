...
# spawn on codex too, primary config/crew-limit bounds how many workers that home
# may run at once (its own bound, not a shared fleet-wide one),
# primary config/backlog-backend=manual makes that home
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness crew-limit backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context}"
