...
- `pr-description-summarize` - load, for a direct-PR ship task, immediately before `gh-axi pr create`; for a no-mistakes ship task, immediately after the pipeline opens the PR and before reporting `done: PR <url> checks green`.
