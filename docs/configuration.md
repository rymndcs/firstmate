...
Each effective `FM_HOME` then contains the four private operational directories below, the optional gitignored `.env` carrying the X-mode pairing token under "X mode (.env)" below, and a gitignored `.no-mistakes/` holding this home's local validation state and evidence.
...
`config/` holds local gitignored operating choices, one file per choice, and each file's format, default, and effect are owned either by its own section below or by the backend reference that section points at, as `config/herdr-presentation-spaces` is owned by [`herdr-backend.md`](herdr-backend.md) and `config/cmux-socket-password` by [`cmux-backend.md`](cmux-backend.md).
`secondmate-provisioning` separately owns which of these files a secondmate home inherits.
...
A herdr task additionally records `herdr_session=`, `herdr_workspace_id=`, `herdr_tab_id=`, and `herdr_pane_id=`, plus the optional presentation-only `herdr_worktree_tab_id=` and `herdr_worktree_pane_id=` written only for a projected task whose extra worktree tab was created and verified.
...
Every profile array is an implicit capacity-aware choice resolved through `quota-array-dispatch` from `dispatch-axi`'s ranked output.
...
An absent, unreadable, or empty file means the binding never calls the network: a task with no resolvable Linear issue is skipped in complete silence, since most firstmate tasks are not Linear-tracked, but a task that DOES resolve to one prints a `LINEAR: no API key configured - move <identifier> to <state> yourself` line so the transition is not just forgotten; see `bin/fm-linear.sh` for the exact contract.
A task's issue is resolved only from the Linear branch name its task record carries, so a task with no such branch is the normal case rather than an error.
...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.31.2 or newer, gh-axi, chrome-devtools-axi, lavish-axi, compatible tasks-axi per "Backlog backend" above, dispatch-axi, and quota-axi v0.1.16 or newer.
This section is the single owner of that universal toolchain list; backend guides' prerequisites point here and add only their backend-specific tools.
In that list, no-mistakes runs the validation pipeline, gh-axi, chrome-devtools-axi, and lavish-axi cover GitHub, browser, and rich-review operations, tasks-axi backs backlog mutations, and dispatch-axi backs capacity-aware array dispatch.
quota-axi is required alongside dispatch-axi rather than instead of it: dispatch-axi reads `quota-axi --json` for every window provider, and firstmate reads `quota-axi auth --json` for per-provider credentials, a surface dispatch-axi does not expose.
...
When a running home advances and its loaded instruction surface (`CLAUDE.md`, `bin/`, or `.agents/skills/`) changed, bootstrap sends the re-read nudge itself through the stable `fm-<id>` selector and reports the exact completed send as `BOOTSTRAP_INFO:`.
...
`fm-teardown.sh` retries Git's `Unable to create '...index.lock': File exists` return failure up to `FM_TREEHOUSE_RETURN_LOCK_RETRIES` times, and that signature is the only return failure these settings govern; [`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the rest of the return-failure contract.
