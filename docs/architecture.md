...
Its optional default-off presentation projection may place one clean new task in a disposable workspace, alongside a best-effort second tab holding an idle shell at that task's own worktree, without changing endpoint authority or lifecycle ownership; [Optional presentation spaces](herdr-backend.md#optional-presentation-spaces) owns that conditional design, the extra tab's create and teardown rules, and its narrow home-local restored-shell cleanup at locked session start.
...
That branch is `fm/<id>` unless the task supplied its own name through `bin/fm-brief.sh --branch`, and the tangle check is branch-shape agnostic either way: it classifies any named non-default primary branch.
...
The dispatch file is intentionally judgment-based: firstmate reads the natural-language rules at intake, chooses the best matching rule, resolves profile arrays itself from current `dispatch-axi` output under the `AGENTS.md` section 4 intake boundary and the `quota-array-dispatch` selection procedure, and passes only concrete `--harness`, `--model`, and `--effort` axes to `fm-spawn.sh`.
...
Durable project-intrinsic agent knowledge lives in each project's committed `CLAUDE.md`, with `AGENTS.md` as a symlink.
Ship briefs prompt crewmates to create or update those files through the normal delivery path; `data/projects.md` stays a thin private registry.
Each project `CLAUDE.md` carries a short `## Maintaining this file` self-governance section; `bin/fm-ensure-agents-md.sh` owns the canonical wording and injects it idempotently when creating the skeleton, promoting an existing `AGENTS.md`, or reconciling an existing `CLAUDE.md` that still lacks it.
It refuses case-variant memory names such as lowercase `agents.md` or `claude.md`, because the `AGENTS.md` symlink must carry the uppercase literal `CLAUDE.md` target portably across case-sensitive and case-insensitive filesystems, and surfaces the mismatch for manual reconciliation.
